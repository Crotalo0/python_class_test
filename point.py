...
class Point:
    ...
    def get_circumference(self, p, q):
        left_side = array([[2 * self.x, 2 * self.y, 1], [2 * p.x, 2 * p.y, 1], [2 * q.x, 2 * q.y, 1]])
        right_side = array([-(self.x ** 2) - (self.y ** 2), -(p.x ** 2) - (p.y ** 2), -(q.x ** 2) - (q.y ** 2)])
        result = linalg.inv(left_side).dot(right_side)
